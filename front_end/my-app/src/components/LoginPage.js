...
const LoginPage = ({ onLogin, setCurrentPage, therapistCredentials }) => {
    ...
    const handleLogin = () => {
        if (loginType === 'therapist') {
            ...
            if (username.trim()) {
                onLogin({ name: username, type: "student" }, 'dashboard');
            }
        }
    };
                ...
                <div className="card-body">
                    ...
                    <div className="nav nav-pills nav-fill mb-4 bg-light rounded-pill p-1">
                        <button
                            ...
                            className={`nav-link rounded-pill fw-medium ${loginType === 'student' ? 'active bg-primary' : 'text-dark'}`}
                        >
                            Student
                        </button>
                        <button
                            onClick={() => setLoginType('therapist')}
                            className={`nav-link rounded-pill fw-medium ${loginType === 'therapist' ? 'active bg-primary' : 'text-dark'}`}
                        >
                            Therapist
                        </button>
                    </div>
                    ...
                    <form onSubmit={(e) => { e.preventDefault(); handleLogin(); }}>
                        <div className="mb-3">
                            <label className="form-label fw-medium">
                                <User className="d-inline me-2" size={20} />
                                {loginType === 'therapist' ? 'Username' : 'Your Name'}
                            </label>
                            <input
                                type="text"
                                value={username}
                                ...
                                placeholder={loginType === 'therapist' ? 'Enter username' : 'Enter your name'}
                                required
                            />
                        </div>
                        ...
                        {loginType === 'therapist' && (
                            <>
                                <div className="mb-3">
                                    <label className="form-label fw-medium">
                                        <Lock className="d-inline me-2" size={20} />
                                        Password
                                    </label>
                                    <input
                                        type="password"
                                ...
                                <div className="alert alert-info small">
                                    ...
                                    Username: therapist123<br />
                                    Password: secure2024
                                </div>
                            </>
                        )}

                        <button type="submit" className="btn btn-primary btn-lg w-100 fw-bold hover-scale mt-3">
                            {loginType === 'therapist' ? 'Access Dashboard' : "Let's Play!"}
                        </button>
                    </form>

                    <div className="text-center mt-4">
                        <button onClick={() => setCurrentPage('signup')} className="btn btn-link">
                            New here? Sign up!
                        </button>
                    </div>
                </div>
