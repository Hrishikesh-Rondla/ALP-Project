...
// const Dashboard = ({ user, score, hearts, studentPerformance, onStartQuiz, onLogout, isEvaluated, showConfetti }) => {
//     // This safety check ensures the component never crashes, even if data is missing.
//     const mediumPerformance = studentPerformance?.medium ?? { correct: 0, total: 0 };
...
// src/components/Dashboard.js (Updated with Lottie Animation)

import React from 'react';
import Confetti from 'react-confetti';
import Lottie from "lottie-react"; // --- IMPORT LOTTIE ---
import { Star, Trophy, Heart, Home, Play, Award, BarChart } from 'lucide-react';
import successAnimation from '../assets/success-animation.json'; // --- IMPORT YOUR ANIMATION FILE ---

// The LevelCard and ProgressDisplay components remain unchanged.
const LevelCard = ({ level, title, description, questions, stars, color, onStart, isEnabled = true }) => (
    <div className="col-md-4 mb-4">
        <div
            className={`card h-100 text-center shadow-sm rounded-4 border-0 ${isEnabled ? 'hover-scale' : 'opacity-50'}`}
                ...
                {!isEvaluated ? (
                    ...
                    <>
                        <div className="card shadow-sm rounded-4 p-4 mb-4 text-center">
                            ...
                            {/* We replace the icon-circle div with the Lottie component */}
                            <div className="mx-auto" style={{ width: '120px', height: '120px', marginTop: '-20px' }}>
                                <Lottie animationData={successAnimation} loop={false} />
                            </div>

                            <h2 className="h3 fw-bold text-dark" style={{ marginTop: '-10px' }}>Evaluation Complete!</h2>
                            <p className="fs-5 text-muted">You scored <strong>{mediumPerformance.correct} out of {mediumPerformance.total}</strong> on the evaluation quiz.</p>
                            <p className="fs-5 fw-bold">Based on your result, we recommend you practice the <span className={`text-${recommendation === 'hard' ? 'danger' : 'success'}`}>{recommendation.toUpperCase()}</span> level.</p>
                        </div>
                        <h3 className="h5 fw-bold text-dark mb-3 text-center">Practice Your Skills</h3>
